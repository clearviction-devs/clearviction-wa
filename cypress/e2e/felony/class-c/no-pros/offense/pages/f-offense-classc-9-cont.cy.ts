...
describe('f-offense-classc-9-cont', () => {
  beforeEach(() => {
    cy.visit('/calculator/f-offense-classc-9-cont');
  });
  ...
  it('directs to f-offense-classc-10-cont when clicking No', () => {
    cy.get('[data-cy="calc-choice-1')
      .should('be.visible')
      .click();
    cy.url().should('include', '/calculator/f-offense-classc-10-cont');
  });
  it('directs to f-offense-classc-9-ineligible when clicking Yes', () => {
    cy.get('[data-cy="calc-choice-0')
      .should('be.visible')
      .click();
    cy.url().should('include', '/calculator/f-offense-classc-9-ineligible');
  });
  testNotSureModal();
  testPreviousButton('f-offense-classc-9-cont');
});
